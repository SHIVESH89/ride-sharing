...
router.get('/', (req, res) => {
    try {
        res.json(allSQL(`SELECT p.*, r.pickup_location, r.drop_location, u.name as passenger_name
...
router.get('/ride/:ride_id', (req, res) => {
    try {
        res.json(allSQL(`SELECT p.*, u.name as passenger_name FROM Payment p
