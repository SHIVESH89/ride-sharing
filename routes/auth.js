...
// POST /api/auth/login — Login with name + phone
router.post('/login', (req, res) => {
    ...
    try {
        const user = getSQL('SELECT * FROM User WHERE name = ? AND phone = ?', [name.trim(), phone.trim()]);
        if (!user) return res.status(401).json({ error: 'No account found with that name and phone number' });

        const isPassenger = getSQL('SELECT user_id FROM Passenger WHERE user_id = ?', [user.user_id]);
        const isDriver = getSQL(`SELECT d.*, v.vehicle_type, v.registration_no FROM Driver d
...
// POST /api/auth/register — Register new passenger or driver
router.post('/register', (req, res) => {
    ...
    try {
        ...
        const existing = getSQL('SELECT * FROM User WHERE phone = ?', [phone.trim()]);
        if (existing) {
            // Phone exists — check if they can add the other role
            if (role === 'passenger') {
                const alreadyPassenger = getSQL('SELECT user_id FROM Passenger WHERE user_id = ?', [existing.user_id]);
                if (alreadyPassenger) return res.status(409).json({ error: 'Already registered as passenger' });
                runSQL('INSERT INTO Passenger (user_id) VALUES (?)', [existing.user_id]);
                return res.status(201).json({ user_id: existing.user_id, name: existing.name, phone: existing.phone, role: 'passenger' });
            } else {
                const alreadyDriver = getSQL('SELECT user_id FROM Driver WHERE user_id = ?', [existing.user_id]);
                if (alreadyDriver) return res.status(409).json({ error: 'Already registered as driver' });
                transaction(() => {
                    runSQL('INSERT INTO Driver (user_id, license_no, status, rating) VALUES (?, ?, ?, ?)', [existing.user_id, license_no, 'Available', 4.0]);
                    runSQL('INSERT INTO Vehicle (vehicle_type, registration_no, driver_user_id) VALUES (?, ?, ?)', [vehicle_type, registration_no, existing.user_id]);
                });
        ...
        const userId = transaction(() => {
            const r = runSQL('INSERT INTO User (name, phone, trust_score) VALUES (?, ?, ?)', [name.trim(), phone.trim(), 5]);
            if (role === 'passenger') {
        ...
        if (err.message.includes('UNIQUE')) return res.status(409).json({ error: 'Phone, license, or registration number already exists' });
